...
JadeEngine.prototype.setup = function(config){
	this.config = config;
}

JadeEngine.prototype.renderFunction = function(template, content, partials, helpers) {

	var content_with_helpers = _.extend({}, content, helpers.tag, helpers.block);

	var p = require('path');
	var compiled_template = Jade.compile(template, { "basedir": p.join(process.cwd(), this.config.template_dir) });
